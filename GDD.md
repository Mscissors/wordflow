...
- There are currently **120 chains** (see `PUZZLES` in `index.html` for the canonical list). This count goes stale every time the array grows; recompute it from `PUZZLES.length` rather than trusting it.
...
- **Avoid ambiguous forks near the top of the chain.** If the anchor is DOG, the second word could be PARK, HOUSE, FOOD, BONE... The player only has letter-count + the free first letter + timer reveals to disambiguate; that's acceptable mid-chain but frustrating at word 2. Prefer anchors whose first link is strongly determined (SECRET → AGENT).
...
- The rotation repeats silently once it runs out. `PUZZLES` holds **120 chains** as of 2026-08-29, so index 0 is served again on **day 121 = 2026-11-15**; extend the array before then to avoid repeats. *(This line has gone stale four times now: "21 chains / 2026-08-08", "42 / 2026-08-29", "60 / 2026-09-16", and "90 / 2026-10-16". Recompute it from `PUZZLES.length` whenever you add chains rather than trusting the prose. Note also that a count here describes the LOCAL file: on 2026-08-29 live was three commits behind and serving only 42, so check production separately before claiming what players see.)*
